...
//	imClient.EnsureUsers(ctx, dim.USER_SYSTEM_NOTICE, demo.USER_A)
//	session, _ := imClient.Login(ctx, dim.USER_SYSTEM_NOTICE.ID)
//	conv, _ := session.Conversations().GetOrCreatePrivate(ctx, demo.USER_A.ID)
//	session.Messages().Send(ctx, conv.ID, NewMessage(TextMessage("通知内容")))
//
//	// 以系统订单身份向用户 Alice 发送卡片消息
//	imClient.EnsureUsers(ctx, dim.USER_SYSTEM_ORDER, demo.USER_A)
//	session, _ := imClient.Login(ctx, dim.USER_SYSTEM_ORDER.ID)
//	conv, _ := session.Conversations().GetOrCreatePrivate(ctx, demo.USER_A.ID)
//	session.Messages().Send(ctx, conv.ID, NewMessage(CardMessage(CardInput{
//	    Title:       "订单已发货",
...
//	system_report       数据报告 —— 周报、月报、销售数据等统计报告k'm'h'h'h'h'h'h'h'h'h'h'yu
package dim
...
var USER_SYSTEM_LOGISTICS = User{
	ID:       "system_logistics",
	Nickname: "物流消息",
...
var USER_SYSTEM_INTERACTION = User{
