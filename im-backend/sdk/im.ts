...
export enum MessageType {
  ...
  export interface Conversation {
    ...
    user_states?: { [key: string]: { last_activated_at?: string; last_read_at?: string; unread_count?: number } };
    created_at: string;
    updated_at: string;
  ...
  class IMSDK {
    ...
    async connect(): Promise<void> {
      try {
        this.ws = new WebSocket(`${this.baseURL.replace('http', 'ws')}/api/ws?token=${this.token}`);
